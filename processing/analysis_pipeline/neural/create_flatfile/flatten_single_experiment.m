function flatten_single_experiment(input_folder)
...
if ~isfolder(input_folder)
    disp("Can't find experiment: " + input_folder + ". Skipping...");
    return;
end

temp = dir(fullfile(input_folder));  % Get the directories
names = {temp.name};
rat_dir = fullfile(input_folder, names{endsWith(names, '_Behavior')});
...
silenced_times = load_silenced_times(input_folder, sampling_rate);
