...
%  CHECK_IF_LEGAL checks the legacy of the given val as a Ramp_comp-object property.
%   [VALID_FLAG,ERR,IS_FORMULA]=CHECK_IF_LEGAL(COMP,PROP,VAL,DEF_ARR,
%   VAR_ARR) checks that VAL is a legal value for the Ramp_comp-object property-PROP.
  ...
  switch prop
       case 'Static_value'
            if (~isempty(numeric))%dealing with a constant number
                   if (~( is_legal_ramp(comp,numeric)))
							valid_flag=0; 
							err='Ramp must be a real positive number';
                     end
