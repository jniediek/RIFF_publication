...
)
{
	...
	case 13:
            ...
            case 24:
                //modified for use by maestro, ELN 12 Nov 2014
              {//send block to the embedded
                if(nrhs<3){
					 mexErrMsgTxt(" 2 inputs required.");						
					 break;
					}
                    int sizeNumericArray[2];
                    sizeNumericArray[1]=mxGetM(prhs[2]);
					sizeNumericArray[0]=mxGetN(prhs[2]);
                    int channelID=mxGetScalar(prhs[1]);
                    //plhs[1]=mxCreateNumericArray(2,sizeNumericArray,mxINT16_CLASS,mxREAL);
                    if(sizeNumericArray[1] != 20000 | sizeNumericArray[0] != 1){
						 mexErrMsgTxt(" wrong Data array");						
						 break;
					}
                    short * arraydata = (short *)mxGetPr(prhs[2]);

                    int  DataCaptured =0;
                    int ret=GetChannelData(channelID,arraydata,sizeNumericArray[1],&DataCaptured); 
                    plhs[1]=mxCreateScalarDouble(DataCaptured);
                    plhs[0]=mxCreateScalarDouble((int)ret);
                    break;
               }
