function file_format_data=get_file_format_vals(signal,signal_table,line_num,chan_num,line_trial_num,...
                                                                     num_lines,line_trials,line_max_trials)
...
s=size(str);
str{s(2)+1}=['TRIAL_DUR_VAL(',st_num1,',',st_num3,')=',num2str(meta.T(line_num,'trial_dur')),';'];
str{s(2)+2}=['LEVEL_VAL(',st_num1,',',st_num2,',',st_num3,')=',num2str(meta.T(line_num,'att')),';'];
str{s(2)+3}=['STIME_VAL(',st_num1,',',st_num2,',',st_num3,')=',num2str(meta.T(line_num,'stime'),';'];
str{s(2)+4}=['ETIME_VAL(',st_num1,',',st_num2,',',st_num3,')=',num2str(meta.T(line_num,'etime'),';'];
str{s(2)+5}=['RAMP_VAL(',st_num1,',',st_num2,',',st_num3,')=',num2str(meta.T(line_num,'ramp'),';'];

%% MAYBE WE DO NEED THIS o_O
...
%                 str{s(2)+1}=[sig_name,'_',upper(name),'_VAL_L',st_num1,'_C',st_num2,'=cell(1',',',num2str(line_trials),');'];
%             end
%         end
%         s=size(str);
%         if isa(comp,'SIG_COMP')
...
%         elseif isa(comp,'STRING_COMP')
%             str{s(2)+1}=[sig_name,'_',upper(name),'_VAL_L',st_num1,'_C',st_num2,'{',st_num3,'}=''',signal_table{k},''';'];
%         end
%     end
% end
