function varargout = line_pst(varargin)
% LINE_PST M-file for line_pst.fig
%      LINE_PST, by itself, creates a new LINE_PST or raises the existing
%      singleton*.
%
%      H = LINE_PST returns the handle to a new LINE_PST or the handle to
%      the existing singleton*.
%
%      LINE_PST('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in LINE_PST.M with the given input arguments.
%
%      LINE_PST('Property','Value',...) creates a new LINE_PST or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before line_pst_OpeningFunction gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to line_pst_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help line_pst

% Last Modified by GUIDE v2.5 01-Jan-2006 16:52:48

% Begin initialization code - DO NOT EDIT
gui_Singleton = 0;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @line_pst_OpeningFcn, ...
                   'gui_OutputFcn',  @line_pst_OutputFcn, ...
                   'gui_CreateFcn', @line_pst_CreateFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before line_pst is made visible.
function line_pst_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to line_pst (see VARARGIN)
global LINE_PST_CREATED;

if (LINE_PST_CREATED==0) 
    a='line_pst_OpeningFcn'
    % setting the screen size
    set(0,'Units','characters');
    scrsz = get(0,'ScreenSize');
    set(handles.line_pst,'Position',[0 scrsz(4)*0.08 scrsz(3)*0.999 scrsz(4)*0.88]);
    set(0,'Units','pixels');
...
% Choose default command line output for line_pst
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes line_pst wait for user response (see UIRESUME)
% uiwait(handles.line_pst);


% --- Outputs from this function are returned to the command line.
function varargout = line_pst_OutputFcn(hObject, eventdata, handles) 
...
% --- Executes during object creation, after setting all properties.
function line_pst_CreateFcn(hObject, eventdata, handles)
% hObject    handle to line_pst (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
% --- Executes during object creation, after setting all properties.
global LINE_PST_CREATED;
...
% --- Executes when user attempts to close line_pst.
function line_pst_CloseRequestFcn(hObject, eventdata, handles)
% hObject    handle to line_pst (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
...
if ~isempty(Out_Manager)
    ...
    LINE_PST_DATA_WAS_RESET(fig_location)=[];
    LINE_PST_APPLY_PRESSED(fig_location)=[];
    LINE_PST_INPUT_WAS_CHANGED(fig_location)=[];
    LINE_PST_TRIAL_DATA_INITIALIZED(fig_location)=[];
...
if   EXP_RUNNING
    ...
    if ~(handles.user_time(cur_panel))
        ...
        elseif change_times
            for panel=panel_range
                ...
                LINE_PST_INPUT_WAS_CHANGED{fig_location}(panel)=1;
            end
    ...
    %electrode range input
     if ~(handles.user_electrode(cur_panel))
        ...
        LINE_PST_INPUT_WAS_CHANGED{fig_location}(cur_panel)=1;
     end
    ...
    for panel=panel_range
        if ((LINE_PST_INPUT_WAS_CHANGED{fig_location}(panel)) || ~(LINE_PST_APPLY_PRESSED{fig_location}(panel)))
            ...
            plot(linspace(handles.x_start(panel),handles.x_end(panel),length(handles.image_data{panel})),...
                    handles.image_data{panel}');
            if init_trial_data(panel)
                %indicates that inputs were changed  besides the time
                %inputs and therefore the data-structure that holds all
                %trials data was initialized
                global LINE_PST_TRIAL_DATA_INITIALIZED;
                LINE_PST_TRIAL_DATA_INITIALIZED{fig_location}(panel)=1;
        ...
        LINE_PST_APPLY_PRESSED{fig_location}(panel)=1;
    end
     ...
     handles_list=get(Out_Manager,'Plot_handles');
       ...
       if ~(handles.user_electrode(cur_panel))
        ...
        if strcmp(electrode_str,'-')
            ...
            handles.electrode(cur_panel)= electrode;
            handles.spk(cur_panel)= spk;
            tmp=handles.electrode;
            handles.user_electrode(cur_panel)=1;
            init_trial_data(cur_panel)=1;
        end
        LINE_PST_INPUT_WAS_CHANGED{fig_location}(cur_panel)=1;
         [Out_Manager,fig_data]=create_plots_after(Out_Manager,LINE_TABLES,handles.electrode(cur_panel),cur_panel,6);
         handles=fig_data; 
         handles.electrode=tmp;
     end
      

    if(LINE_PST_DATA_WAS_RESET(fig_location)==1)%the metablock ran at least once
...
%             line_trials=get(line,'Line_num_of_trials');
            ...
            if handles.user_time_range(panel)
                plot(linspace(handles.x_start(panel),handles.x_end(panel),length(img)),img');
                axis([handles.x_start(panel) handles.x_end(panel) handles.color_start(panel),...
               handles.color_end(panel)]); 
            else
                plot(linspace(handles.x_start(panel),handles.x_end(panel),length(img)),img');
                    cl=get(handles.axes_list(panel),'YLim');
   ...
   end%if( LINE_PST_DATA_WAS_RESET(fig_location)==1)
end%if   EXP_RUNNING
h=handles;
