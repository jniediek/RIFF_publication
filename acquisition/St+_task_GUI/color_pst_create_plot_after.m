% COLOR_PST_CREATE_PLOT is the display function of color_pst.
% COLOR_PST_CREATE_PLOT(FIG_DATA,VALUES_TABLE,NPTS,TOTAL_DATA,
% BUF_SAMP_RATE,P_HANDLE,PLOT_INDEX)
% display the image in respect to the data gathered while the experiment is
% running.
% FIG_DATA - the color_pst application data
...
function  fig_data=color_pst_create_plot_after(fig_data,values_table,npts,panel_num,total_data,buf_samp_rate,p_handle,plot_index,electrode_num)
global Out_Manager;
...
%     end
    fig_data.electrode(k)=electrode_num;
    
%     if (~(COLOR_PST_APPLY_PRESSED{plot_index}(k)))%since this is a new run and the user must press apply
...
%     if ~(fig_data.user_time_range(k))
        ...
        if fig_data.x_end_npts(k)==0
            ...
            COLOR_PST_INPUT_WAS_CHANGED{plot_index}(k)=1;
            fig_data.user_time_range(k)=1;
%     end
   
    collected_trial=get(Out_Manager,'Collected_trial');
    all_trials_data=get(Out_Manager,'Trial_data');
    if COLOR_PST_INPUT_WAS_CHANGED{plot_index}(k)
        COLOR_PST_INPUT_WAS_CHANGED{plot_index}(k)=0;
                ...
                for tr=1:length(line_loc)
                      trial_loc=trial_num_in_line(line_loc(tr));  
                       comp1_val=table{comp1_table_entry,trial_loc};%value of comp1 of this trial
                        ...
                        fig_data.image_data{k}(entry1,:)=fig_data.image_data{k}(entry1,:)+trial_data(range);
                        fig_data.image_data_num_trials{k}(entry1)=fig_data.image_data_num_trials{k}(entry1)+1;
%                            if COLOR_PST_TRIAL_DATA_INITIALIZED{plot_index}(k)%inputs were changed beside the time input
                            fig_data.total_trials_data{k}(entry1,:)=fig_data.total_trials_data{k}(entry1,:)+trial_data;
                            fig_data.total_trials_data_num_trials{k}(entry1)=fig_data.total_trials_data_num_trials{k}(entry1)+1;
%                           end
%                          else 
%                              fig_data.total_trials_data{k}=fig_data.image_data{k};
%                          end
                end%for
            end%if ~isempty(line_loc)
        end%if ~(collected_trial==1)
    end%if COLOR_PST_INPUT_WAS_CHANGED{plot_index}(k)
    COLOR_PST_TRIAL_DATA_INITIALIZED{plot_index}(k)=0;
    ...
    img(t2u,:)=img(t2u,:)./fig_data.image_data_num_trials{k}(t2u,ones(size(range)));
    if fig_data.user_color_range(k)
        imagesc([ceil(fig_data.x_start_npts(k)),ceil(fig_data.x_end_npts(k))],[1,fig_data.num_data1(k)],...
            img,[fig_data.color_start(k),fig_data.color_end(k)]);  
        
    else
        imagesc([ceil(fig_data.x_start_npts(k)),ceil(fig_data.x_end_npts(k))],[1,fig_data.num_data1(k)],img);
