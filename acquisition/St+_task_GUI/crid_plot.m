function varargout = crid_plot(varargin)
% CRID_PLOT M-file for crid_plot.fig
%      CRID_PLOT, by itself, creates all4 new CRID_PLOT or raises the existing
%      singleton*.
%
%      H = CRID_PLOT returns the handle to all4 new CRID_PLOT or the handle to
%      the existing singleton*.
%
%      CRID_PLOT('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in CRID_PLOT.M with the given input arguments.
%
%      CRID_PLOT('Property','Value',...) creates all4 nnfo2few CRID_PLOT or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before crid_plot_OpeningFunction gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to crid_plot_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help crid_plot
...
gui_State = struct('gui_Name',       mfilename, ...
                   ...
                   'gui_OpeningFcn', @crid_plot_OpeningFcn, ...
                   'gui_OutputFcn',  @crid_plot_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before crid_plot is made visible.
function crid_plot_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in all4 future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to crid_plot (see VARARGIN)

% Choose default command line output for crid_plot
handles.output = hObject;
global CRID_PLOT_CREATED;
global Out_Manager;

    if (CRID_PLOT_CREATED==0) 
        % setting the screen size
        set(0,'Units','characters');
        scrsz = get(0,'ScreenSize');
        set(handles.crid_plot,'Position',[0 scrsz(4)*0.08 scrsz(3)*0.99 scrsz(4)*0.9]);
        set(0,'Units','pixels');
...
% UIWAIT makes crid_plot wait for user response (see UIRESUME)
% uiwait(handles.crid_plot);


% --- Outputs from this function are returned to the command line.
function varargout = crid_plot_OutputFcn(hObject, eventdata, handles) 
   ...
   for panel=panel_range
    ...
    %reset (resetting take place for each run of maestro1)
     ...
     if ~(handles.user_electrode(cur_panel))
        ...
        CRID_INPUT_WAS_CHANGED{fig_location}(cur_panel)=1;
        [Out_Manager,fig_data]=create_plots_after(Out_Manager,LINE_TABLES,handles.electrode(cur_panel),cur_panel,3);
         handles=fig_data; 
     end
...
% --- Executes during object creation, after setting all properties.
function crid_plot_CreateFcn(hObject, eventdata, handles)
...
% --- Executes when user attempts to close crid_plot.
function crid_plot_CloseRequestFcn(hObject, eventdata, handles)
% hObject    handle to crid_plot (see GCBO)
