...
function varargout = RatTracker(varargin)
    % RatTracker MATLAB code for RatTracker.fig
    %      RatTracker, by itself, creates a new RatTracker or raises the existing
    %      singleton*.
    %
    %      H = RatTracker returns the handle to a new RatTracker or the handle to
    %      the existing singleton*.
    %
    %      RatTracker('CALLBACK',hObject,eventData,handles,...) calls the local
    %      function named CALLBACK in RatTracker.M with the given input arguments.
    %
    %      RatTracker('Property','Value',...) creates a new RatTracker or raises the
    %      existing singleton*.  Starting from the left, property value pairs are
    %      applied to the GUI before RatTracker_OpeningFcn gets called.  An
    %      unrecognised property name or invalid value makes property application
    %      stop.  All inputs are passed to RatTracker_OpeningFcn via varargin.
    %
    ...
    % Edit the above text to modify the response to help RatTracker

    % Last Modified by GUIDE v2.5 08-Sep-2017 00:42:35

    % Begin initialization code - DO NOT EDIT
    gui_Singleton = 1;
    gui_State = struct('gui_Name',       mfilename, ...
                       'gui_Singleton',  gui_Singleton, ...
                       'gui_OpeningFcn', @RatTracker_OpeningFcn, ...
                       'gui_OutputFcn',  @RatTracker_OutputFcn, ...
                       'gui_LayoutFcn',  [] , ...
                       'gui_Callback',   []);
...
function RatTracker_OpeningFcn(hObject, ~, handles, varargin)
...
function varargout = RatTracker_OutputFcn(~, ~, handles)
...
%     handles = helper('rec', handles); % change button color to black
	...
	while handles.filming.is_recording
        ...
        for j = 1:length(t)   % For each frame that are acquired in this bulk (most likely 1)
            ...
            m = mean(mean(frame));
            if((m < 20) || (m > 100)) % The frame is too bright (main light was set on) or too dark. Skip data analysis
            ...
            for i =1:num_of_rats
                ...
                locs_arr(i*2-1) = -1*rat_handlers{i}.loc(1);    % By convention, X should be sent with negative sign.
            ...
            if(handles.filming.is_linked) % Try to send over TCP/IP only if handshake was peformed
                fwrite(handles.filming.h_maestro, locs_arr, 'short'); % location is always a positive value
            end
            %   ========== Save frame to .tiff ===================
...
function connect_button_Callback(hObject, ~, handles)
    ...
    is_linked = 'true';
    h_maestro = tcpip('132.64.105.163', 80, 'NetworkRole', 'client');
    try
        fopen(h_maestro);
        pause(0.2);
...
function custom_button_Callback(hObject, ~, handles)
    ...
    if(1)
%         handles.filming.src.Exposure = 0.01;
        1;
    end
    if(0)
        handles.filming.is_recording = false;
