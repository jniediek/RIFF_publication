...
while length(property_argin) >= 2,
    ...
    if ~isa(prop,'char')
       treat_error(['The property input is not a valid Level property'])
    end 
		...
		case 'Sig_comp'
            ...
            case 'Seq_values'
				if (~isa(val,'double') || isempty(val))
				    treat_error('The given value is not a legal sequence for Level component'); 
				else
                    for k=1:length(val)
				        if ~(is_legal_level(comp,val(k)))
                             treat_error(['The given value : ',num2str(val(k)),' is not a legal Level']); 
                         end
                     end%for
            ...
            case 'Sweep'
				if (~isa(val,'Sweep'))  
                    ...
                    sdata=get(val,'Sdata');
                    edata=get(val,'Edata');
                    if  (~is_legal_level(comp,sdata) || ~is_legal_level(comp,edata))
                        treat_error('The given value is not a legal Level'); 
                    else
