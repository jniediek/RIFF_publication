...
%  CHECK_IF_LEGAL checks the legacy of the given val as a Trial_dur_comp-object property.
%   [VALID_FLAG,ERR,IS_FORMULA]=CHECK_IF_LEGAL(COMP,PROP,VAL,DEF_ARR,
%   VAR_ARR) checks that VAL is a legal value for the Trial_dur_comp-object property-PROP.
...
switch prop
    case 'Static_value'
         if (~isempty(numeric))%dealing with a constant number
                  if (~( is_legal_trial_dur(comp,numeric)))
                        valid_flag=0; 
                        err='Trial duration must be in the range [1,4000]';
                  end
                      
        else%dealing with a potential formula
                  [is_formula,final_value,def_exist]=is_legal_formula(val,def_arr,var_arr);
                  if (is_formula)
                          if (final_value<0 || final_value==0 || final_value>3999) 
                                valid_flag=0; 
                                err='Trial duration must be in the range [1,4000]';
                          end
                  end
