...
function  color_pst_create_plot(fig_data,values_table,npts,total_data,buf_samp_rate,p_handle,plot_index)
global Out_Manager;
...
disp(['npts=' num2str(npts) ', buf_size=' num2str(buf_size)]);
...
for k=1:4
        ...
        if ~(collected_trial==1)
            ...
            if ~isempty(line_loc)%if the line ran atleast once before
                for tr=1:length(line_loc)
                    ...
                    fig_data.image_data{k}(entry1,1:length(range))=fig_data.image_data{k}(entry1,1:length(range))+trial_data(range);
    ...
    if size(trial_data,2)>buf_size
        trial_data=trial_data(:,1:buf_size);
    end
    ...
    if fig_data.user_color_range(k)
        imagesc([fig_data.x_start(k),fig_data.x_end(k)],[1,fig_data.num_data1(k)],...
            img,[fig_data.color_start(k),fig_data.color_end(k)]);
        
    else
        imagesc([fig_data.x_start(k),fig_data.x_end(k)],[1,fig_data.num_data1(k)],img);
