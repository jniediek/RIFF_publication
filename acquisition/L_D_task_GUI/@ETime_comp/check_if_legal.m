...
%  CHECK_IF_LEGAL checks the legacy of the given val as a ETime_comp-object property.
%   [VALID_FLAG,ERR,IS_FORMULA]=CHECK_IF_LEGAL(COMP,PROP,VAL,DEF_ARR,
%   VAR_ARR) checks that VAL is a legal value for the ETime_comp-object property-PROP.
...
if (~isempty(numeric))%dealing with a constant number
		if (~( is_legal_etime(comp,numeric)))
		    valid_flag=0; 
