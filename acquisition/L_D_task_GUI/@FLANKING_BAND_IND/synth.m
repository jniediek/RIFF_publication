...
switch nargin
    case 4
        ...
        if ~(isempty(params))
            if ~(length(params)==2)
                treat_error('Wrong input argument to FLANKING_BAND_IND/synth');
            end
            freq=params{1};
            bw=params{2};
        else
        ...
        % if we do not want co-modulated noise, we have to generate
        % a second gaussian noise
            gnoise2=randn(lpsn,1);
            filtered2=filter(sig.filtercoeffs,1,gnoise2);

        % upsample the noise to the actual sampling rate
        filtered1=interp1(1:lpsn,filtered1,linspace(1,lpsn,samplenum));
            filtered2=interp1(1:lpsn,filtered2,linspace(1,lpsn,samplenum));
        ...
        % multiply filtered noise with sinosoid and add them up
            samp_points=filtered1.*s1+filtered2.*s2;
