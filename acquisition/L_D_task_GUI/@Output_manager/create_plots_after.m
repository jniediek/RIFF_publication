...
function  [out_manager,fig_data]=create_plots_after(out_manager,line_tables,electrode_num,panel_num,plot_num,crid)
...
%                 elseif ((trial_index>2) && (plot_num==4))
            ...
            elseif plot_num==3 %plot of type crid_plot
             ...
             elseif plot_num==6 %plot of type color_pst 
             ...
             if ~(global_reset(q)) % if the relevant graphic-window wasent reset yet
                    ...
                    elseif ((plot_num==3) || (plot_num==5) || (plot_num==6)) %plot of type crid_plot or color_pst
                        %comp_list and comp_entries must be the same size
                        %(comp_entries holds for each comp it's table entry)
...
%                if ((trial_index>2) && (plot_num==2)) %plot of type comp_plot
               if ((plot_num==2)) %plot of type comp_plot
                    comp_create_plot_after(fig_data,line_tables,out_manager.buf_rate,p_handle,q,electrode_num,panel_num);
%                elseif ((trial_index>2) && (plot_num==3)) %plot of type crid_plot
              elseif ((plot_num==3)) %plot of type crid_plot
                   crid_create_plot_after(fig_data,line_tables,out_manager.buf_rate,p_handle,q,electrode_num,panel_num);
%                elseif ((trial_index>2) && (plot_num==5)) %plot of type color_pst
              elseif ((plot_num==5)) %plot of type color_pst
                   npts=get(out_manager,'Cur_trial_npts');
                   fig_data.electrode=electrode_num;
                   fig_data=color_pst_create_plot_after(fig_data,line_tables,npts,panel_num,out_manager.data,...
                   out_manager.buf_rate,p_handle,q,electrode_num);
%                elseif ((trial_index>2) && (plot_num==6)) %plot of type color_pst
              elseif ((plot_num==6)) %plot of type color_pst
                   npts=get(out_manager,'Cur_trial_npts');
                   line_pst_create_plot_after(fig_data,line_tables,npts,out_manager.data,...
                   out_manager.buf_rate,p_handle,q,electrode_num,panel_num);
               end
