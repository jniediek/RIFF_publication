...
for day = 1:4
    ...
    ubn = ui(uts:ute);
    snames = unique(data(day).sound_names);
    ...
    for ii = 1:length(tsounds)
        ...
        sidx = find(snames == data(day).sound_names(ii+5)); %sounds_table.soundname(ii+5)
    ...
    xlim([0,xlimsize(day)/1000]);
    title(['unit #',num2str(unitnum(day))])
    clear N
    ...
    subplot(2,4,day+4)
    hold all;    
    plot(smooth(N(sounds(day),:),11),'k');
    plot(t,data(day).sound-2,'color',[1 1 1]*0.3);
    line([500 500],[0 max(N(sounds(day),:))+0.1],'Color','r','LineWidth',2)    
...
binsize = 12;
unitnum = {'207','302','602','102'}; % we eventually didn't use the data from R5,290620, unit #1, or R7,290420, u #8
figure('Position', [100, 100, 2000, 600]);
